...
interface MeetingChatReplyEventData {
  meetingId: string;
  messageId: string;
  text: string;
  userId: string;
...
const buildMeetingChatInstructions = (
...
export const meetingChatReply = inngest.createFunction(
  {
    id: "meetings/chat.reply",
    idempotency: "event.data.messageId",
  },
  { event: "meetings/chat.reply" },
  async ({ event, step }) => {
    const { meetingId, messageId, text, userId } = event.data as MeetingChatReplyEventData;

    const context = await step.run("load-chat-context", async () => {
      const [existingMeeting] = await db
        .select()
        .from(meetings)
        .where(and(eq(meetings.id, meetingId), eq(meetings.status, "completed")));

      if (!existingMeeting) {
        throw new Error("Meeting not found or not completed");
      }
    ...
    if (userId === context.existingAgent.id) {
      return { skipped: true };
    }

    const GPTResponseText = await step.run("generate-chat-reply", async () => {
      ...
      const previousMessages = channel.state.messages
        ...
        .map<ChatCompletionMessageParam>((message) => ({
          role: message.user?.id === context.existingAgent.id ? "assistant" : "user",
      ...
      const GPTResponse = await openaiClient.chat.completions.create({
        messages: [
          {
            role: "system",
            content: buildMeetingChatInstructions(
              context.existingMeeting.summary,
              context.existingAgent.instructions,
            ),
          },
          ...previousMessages,
    ...
    await step.run("send-chat-reply", async () => {
      const avatarUrl = generateAvatarUri({
        seed: context.existingAgent.name,
        variant: "botttsNeutral",
      });

      await streamChat.upsertUser({
        id: context.existingAgent.id,
        name: context.existingAgent.name,
        image: avatarUrl,
      });
